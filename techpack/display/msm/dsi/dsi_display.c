...
error:
	...
	if ((!strcmp(dsi_display->display_type, "primary")) && (panel->panel_serial_number == 0) &&
		...
		if (strcmp(panel->name, "samsung dsc cmd mode oneplus dsi panel") == 0) {
			...
			if ((register1[0] != 0x9c) || (register2[0] != 0x0a)) {
				...
				DSI_ERR("%s:black_count=%d, greenish_count=%d, total=%d\n",
				       ...
				       esd_greenish_count, esd_black_count + esd_greenish_count);
				rc = -1;
			} else {
				rc = 1;
			}
			...
			DSI_ERR("0xE9 = %02x, %02x, %02x, %02x, 0x0A = %02x\n",
			       ...
			       register1[3], register2[0]);
			if (((register1[3] != 0x00) && (register1[3] != 0x02) &&
			      (register1[3] != 0x06) && (register1[3] != 0x04)) || (register2[0] != 0x9C)) {
			...
			rc = dsi_display_read_panel_reg(display, 0xED,
							register4, 1);
			if (rc < 0)
				goto exit;
			...
			DSI_ERR("0x0A = %02x, 0xB6 = %02x, 0xA2 = %02x, %02x, %02x, %02x, %02x\n",
			       ...
			       register3[1], register3[2], register3[3], register3[4]);

			if ((register1[0] != 0x9c) || (register2[0] != 0x0a) ||
			      (register3[0] != 0x12) || (register3[1] != 0x00) ||
