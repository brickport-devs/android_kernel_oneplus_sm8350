...
void dsi_panel_update_gamma_change_write(struct dsi_panel *panel)
{
	...
	if (strcmp(panel->name, "samsung amb655x fhd cmd mode dsc dsi panel") ==
	    0) {
		/* GAMMA change Setting (120Hz) 80-40-20-10*/
		for (i = 0; i < 229; i++) {
			DSI_ERR("120HZ B9_register_value[%d] = 0x%02x", i,
				b9_register_value_500step[i]);
		}
		for (i = 0; i < 186; i++) {
		...
		r_3 = ((dimming_gamma_120hz[8] & 0x3F) << 4) |
		      ((dimming_gamma_120hz[9] & 0xF0) >> 4);
		if (r_3 < 0) {
			r_3 = 0;
			DSI_ERR("80nit 120hz: r_3 = 0");
		}
		g_3 = ((dimming_gamma_120hz[9] & 0x0F) << 6) |
		      ((dimming_gamma_120hz[10] & 0xFC) >> 2);
		if (g_3 < 0) {
			g_3 = 0;
			DSI_ERR("80nit 120hz: g_3 = 0");
		}
		b_3 = ((dimming_gamma_120hz[10] & 0x03) << 8) |
		      dimming_gamma_120hz[11];
		if (b_3 < 0) {
			b_3 = 0;
		...
		dimming_gamma_120hz[0] =
			(dimming_gamma_120hz[0] & 0xC0) | (r_1 >> 4);
		...
		dimming_gamma_120hz[4] =
			(dimming_gamma_120hz[4] & 0xC0) | (r_2 >> 4);
		dimming_gamma_120hz[5] = ((r_2 & 0x00F) << 4) | (g_2 >> 6);
		dimming_gamma_120hz[6] = ((g_2 & 0x03F) << 2) | (b_2 >> 8);
		dimming_gamma_120hz[7] = b_2 & 0x0FF;
		for (i = 0; i < 8; i++) {
			DSI_ERR("dimming_gamma_120hz_80nit[%d] = 0x%02x", i,
				dimming_gamma_120hz[i]);
		}
		for (i = 0; i < 12; i++)
			dimming_gamma_120hz[12 + i] =
				b9_register_value[131 + i];
		r_3 = ((dimming_gamma_120hz[20] & 0x3F) << 4) |
		      ((dimming_gamma_120hz[21] & 0xF0) >> 4);
		if (r_3 < 0) {
			r_3 = 0;
			DSI_ERR("40nit 120hz: r_3 = 0");
		}
		g_3 = ((dimming_gamma_120hz[21] & 0x0F) << 6) |
		      ((dimming_gamma_120hz[22] & 0xFC) >> 2);
		if (g_3 < 0) {
			g_3 = 0;
			DSI_ERR("40nit 120hz: g_3 = 0");
		}
		b_3 = ((dimming_gamma_120hz[22] & 0x03) << 8) |
		      dimming_gamma_120hz[23];
		if (b_3 < 0) {
			b_3 = 0;
		...
		dimming_gamma_120hz[12] =
			(dimming_gamma_120hz[12] & 0xC0) | (r_1 >> 4);
		dimming_gamma_120hz[13] = ((r_1 & 0x00F) << 4) | (g_1 >> 6);
		dimming_gamma_120hz[14] = ((g_1 & 0x03F) << 2) | (b_1 >> 8);
		dimming_gamma_120hz[15] = b_1 & 0x0FF;
		dimming_gamma_120hz[16] =
			(dimming_gamma_120hz[16] & 0xC0) | (r_2 >> 4);
		dimming_gamma_120hz[17] = ((r_2 & 0x00F) << 4) | (g_2 >> 6);
		dimming_gamma_120hz[18] = ((g_2 & 0x03F) << 2) | (b_2 >> 8);
		dimming_gamma_120hz[19] = b_2 & 0x0FF;
		for (i = 12; i < 20; i++) {
			DSI_ERR("dimming_gamma_120hz_40nit[%d] = 0x%02x", i,
				dimming_gamma_120hz[i]);
		}
		for (i = 0; i < 12; i++) {
			dimming_gamma_120hz[24 + i] = b9_register_value[88 + i];
		}
		r_3 = ((dimming_gamma_120hz[32] & 0x3F) << 4) |
		      ((dimming_gamma_120hz[33] & 0xF0) >> 4);
		if (r_3 < 0) {
			r_3 = 0;
			DSI_ERR("20nit 120hz: r_3 = 0");
		}
		g_3 = ((dimming_gamma_120hz[33] & 0x0F) << 6) |
		      ((dimming_gamma_120hz[34] & 0xFC) >> 2);
		if (g_3 < 0) {
			g_3 = 0;
			DSI_ERR("20nit 120hz: g_3 = 0");
		}
		b_3 = ((dimming_gamma_120hz[34] & 0x03) << 8) |
		      dimming_gamma_120hz[35];
		if (b_3 < 0) {
			b_3 = 0;
		...
		dimming_gamma_120hz[24] =
			(dimming_gamma_120hz[24] & 0xC0) | (r_1 >> 4);
		dimming_gamma_120hz[25] = ((r_1 & 0x00F) << 4) | (g_1 >> 6);
		dimming_gamma_120hz[26] = ((g_1 & 0x03F) << 2) | (b_1 >> 8);
		dimming_gamma_120hz[27] = b_1 & 0x0FF;
		dimming_gamma_120hz[28] =
			(dimming_gamma_120hz[28] & 0xC0) | (r_2 >> 4);
		dimming_gamma_120hz[29] = ((r_2 & 0x00F) << 4) | (g_2 >> 6);
		dimming_gamma_120hz[30] = ((g_2 & 0x03F) << 2) | (b_2 >> 8);
		dimming_gamma_120hz[31] = b_2 & 0x0FF;
		for (i = 24; i < 32; i++) {
			DSI_ERR("dimming_gamma_120hz_20nit[%d] = 0x%02x", i,
				dimming_gamma_120hz[i]);
		}
		for (i = 0; i < 12; i++)
			dimming_gamma_120hz[36 + i] = b9_register_value[45 + i];
		r_3 = ((dimming_gamma_120hz[44] & 0x3F) << 4) |
		      ((dimming_gamma_120hz[45] & 0xF0) >> 4);
		if (r_3 < 0) {
			r_3 = 0;
			DSI_ERR("10nit 120hz: r_3 = 0");
		}
		g_3 = ((dimming_gamma_120hz[45] & 0x0F) << 6) |
		      ((dimming_gamma_120hz[46] & 0xFC) >> 2);
		if (g_3 < 0) {
			g_3 = 0;
			DSI_ERR("10nit 120hz: g_3 = 0");
		}
		b_3 = ((dimming_gamma_120hz[46] & 0x03) << 8) |
		      dimming_gamma_120hz[47];
		if (b_3 < 0) {
			b_3 = 0;
		...
		dimming_gamma_120hz[36] =
			(dimming_gamma_120hz[36] & 0xC0) | (r_1 >> 4);
		dimming_gamma_120hz[37] = ((r_1 & 0x00F) << 4) | (g_1 >> 6);
		dimming_gamma_120hz[38] = ((g_1 & 0x03F) << 2) | (b_1 >> 8);
		dimming_gamma_120hz[39] = b_1 & 0x0FF;
		dimming_gamma_120hz[40] =
			(dimming_gamma_120hz[40] & 0xC0) | (r_2 >> 4);
		dimming_gamma_120hz[41] = ((r_2 & 0x00F) << 4) | (g_2 >> 6);
		dimming_gamma_120hz[42] = ((g_2 & 0x03F) << 2) | (b_2 >> 8);
		dimming_gamma_120hz[43] = b_2 & 0x0FF;
		for (i = 36; i < 44; i++) {
			DSI_ERR("dimming_gamma_120hz_10nit[%d] = 0x%02x", i,
				dimming_gamma_120hz[i]);
		}
		/* GAMMA change Setting (120Hz) 80-500*/
		for (i = 0; i < 8; i++) {
			dimming_gamma_120hz_500step[i] =
				b9_register_value_500step[221 + i];
		}
		r_3 = ((dimming_gamma_120hz_500step[0] & 0x3F) << 4) |
		      ((dimming_gamma_120hz_500step[1] & 0xF0) >> 4);
		if (r_3 < 0) {
			r_3 = 0;
			DSI_ERR("120hz: r_3 = 0");
		}
		g_3 = ((dimming_gamma_120hz_500step[1] & 0x0F) << 6) |
		      ((dimming_gamma_120hz_500step[2] & 0xFC) >> 2);
		if (g_3 < 0) {
			g_3 = 0;
			DSI_ERR("120hz: g_3 = 0");
		}
		b_3 = ((dimming_gamma_120hz_500step[2] & 0x03) << 8) |
		      dimming_gamma_120hz_500step[3];
		if (b_3 < 0) {
			b_3 = 0;
			DSI_ERR("120hz: b_3 = 0");
		}
		r_4 = ((dimming_gamma_120hz_500step[4] & 0x3F) << 4) |
		      ((dimming_gamma_120hz_500step[5] & 0xF0) >> 4);
		if (r_4 < 0) {
			r_4 = 0;
			DSI_ERR("120hz: r_4 = 0");
		}
		g_4 = ((dimming_gamma_120hz_500step[5] & 0x0F) << 6) |
		      ((dimming_gamma_120hz_500step[6] & 0xFC) >> 2);
		if (g_4 < 0) {
			g_4 = 0;
			DSI_ERR("120hz: g_4 = 0");
		}
		b_4 = ((dimming_gamma_120hz_500step[6] & 0x03) << 8) |
		      dimming_gamma_120hz_500step[7];
		if (b_4 < 0) {
			b_4 = 0;
			DSI_ERR(" 120hz: b_4 = 0");
		}
		...
		dimming_gamma_120hz_500step[0] =
			(dimming_gamma_120hz_500step[0] & 0xC0) | (r_3 >> 4);
		dimming_gamma_120hz_500step[1] =
			((r_3 & 0x00F) << 4) | (g_3 >> 6);
		dimming_gamma_120hz_500step[2] =
			((g_3 & 0x03F) << 2) | (b_3 >> 8);
		dimming_gamma_120hz_500step[3] = b_3 & 0x0FF;
		dimming_gamma_120hz_500step[4] =
			(dimming_gamma_120hz_500step[4] & 0xC0) | (r_4 >> 4);
		dimming_gamma_120hz_500step[5] =
			((r_4 & 0x00F) << 4) | (g_4 >> 6);
		dimming_gamma_120hz_500step[6] =
			((g_4 & 0x03F) << 2) | (b_4 >> 8);
		dimming_gamma_120hz_500step[7] = b_4 & 0x0FF;
		for (i = 0; i < 8; i++) {
			DSI_ERR("dimming_gamma_120hz_500step[%d] = 0x%02x", i,
				dimming_gamma_120hz_500step[i]);
		}
	}
}
